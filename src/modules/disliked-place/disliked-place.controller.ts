...
export class DislikedPlaceController {
  ...
  @Post('add')
  async addDislikedPlace(@Body() createDislikedPlaceDto: CreateDislikedPlaceDto) {
    const dislikedPlace = await this.dislikedPlaceService.addDislikedPlace(createDislikedPlaceDto);
    return {
